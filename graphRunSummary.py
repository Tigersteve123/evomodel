...
for file in os.listdir(directory):
	...
	runsArr.append((lstCumAvg[-1][0], lstCumAvg[-1][1], len(run), int(tc)/1500000, round(float(acc), 2), np.sum(data[2]), np.average(data[8]), np.sum([run[i]*i for i in range(len(run))])/np.sum(data[3]))) #ending average beta, ending average gamma, total length, capacity, accuracy, total infected, average run length, calculated weighted average run length
...
for i in [runs50, runs70, runs95]:
	...
	summ.plot(ax5, i[3], i[7], 'test capacity', 'weighted average length', style='plot', savedirec=savedirec, legend=['50%', '70%', '95%'])
